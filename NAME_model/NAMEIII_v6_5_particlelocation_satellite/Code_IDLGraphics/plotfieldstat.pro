pro plotfieldstat,datadir,selgrid,selspecies=selspecies,$
    ...
    titletext=titletext,SelTimeAverage=SelTimeAverage,fieldunit=fieldunit,$
    ...
    placenames=placenames,origin=origin,automap=automap,statistic=statistic,$
    plottitle=plottitle
    
  ;--------------------------------------------------------------------
  ; Procedure to plot field mean or maximum (based on Plotfield - DBR 2002)
  ;
  ;
  ; LH March 2010
  ;
  ...
  ;  statistic    : (string) 'mean' or 'maximum' or 'total'
  ;  fieldunit    : (string) to be used in legend instead of units from input file
  ;  plottitle    : (string) set to 'species', 'level', 'field' or 'timeave'
  ...
  if(n_elements(statistic) eq 0) then statistic='mean'
  if(n_elements(fieldunit) eq 0) then fieldunit=''
          ...
          for ifield=startfield,endfield-1 do begin
            ...
            for it=0,numfiles-1 do begin
              ...
              if(it eq 0)then begin
                ...
                if (statistic eq 'mean') then begin
                  fieldstat=fieldstat+dat1
                endif else if (statistic eq 'maximum') then begin
                  index=where(dat1 gt fieldstat,nindex)
                  if(nindex gt 0)then fieldstat(index)=dat1(index)
                endif else if (statistic eq 'total') then begin
                  fieldstat=fieldstat+dat1
                endif
              endelse
              ...
              if (namever eq 3)then begin
                ...
                if(back ne 0)then begin
                  ...
                  if (plottitle eq 'species') then begin
                    postitle=head1(3)
                  endif else if (plottitle eq 'field') then begin
                    postitle=head1(2)
                  endif else if (plottitle eq 'level') then begin
                    postitle=head1(14)
                  endif else if (plottitle eq 'timeave') then begin
                    postitle=head1(7)
                  endif else begin
            ...
            if (statistic eq 'mean') then begin            
              dat1=fieldstat/numfiles
            endif else if (statistic eq 'maximum') then begin
              dat1=fieldstat
            endif else if (statistic eq 'total') then begin
              dat1=fieldstat
            endif
            fieldstat=fieldstat*0.0
             
            if (fieldunit eq '') then begin 
                units=head1(4)
            endif else begin
                units=fieldunit(ifield)
            endelse 
            
            plotfieldpos,lon1,lat1,dat1,position=position,type=type,$
              zoom=zoom,contourlevels=contours1,contourcolors=contourcolors,$
              xyrel=xyrel,fieldunits=units,multiplot=multiplot,$
