pro plotfield,datadir,selgrid,selspecies=selspecies,$
    ...
    plotloc=plotloc,type=type,gengif=gengif,genjpg=genjpg,genpng=genpng,$
    ...
    tekcolors=tekcolors,mapfeatures=mapfeatures,legmax=legmax
  ...
  ;  genpng       : integer (0 or 1) =1 generates png images (on unix)
  ...
  ;                 =4 like 1 but includes validity time on the back map
  ...
  ;                 This allows a much larger spectrum of colour table. Min
  ;                 advised=2 (black,white) 
  ...
  ;
  ; example call for NAME III file: 'Fields_grid1_C1_T1_200901010000.txt'
  ;  GRAPHDIR=/home/h03/apdg/NameIII/NameIIIDevVersion/Version5_3a/Code_IDLGraphics
  ;  WORKDIR=${DATADIR}/${RUNNAME}
  ;  cd $GRAPHDIR
  ;  idl<<eof
  ;  selspecies=['Tracer']
  ...
  ;  plotfield,'${WORKDIR}',selgrid,/exact,$
  ;    selspecies=selspecies,selfield=selfield,sellevel=sellevel,$
  ;    plotpmsl=0,plotuv=0,group='time',gengif=0,$
  ;    multiplot=[0,1,1],plotheader=1,plotlegend=1,genanim=1,$
  ;    projection=8,namever=3
  ;  eof
  ;
  ; example call for NAME II file: 'Fields_grid1_200901010000.txt'
  ;  GRAPHDIR=/home/h03/apdg/NameIII/NameIIIDevVersion/Version5_3a/Code_IDLGraphics
  ;  WORKDIR=${DATADIR}/${RUNNAME}
  ;  cd $GRAPHDIR
  ;  idl<<eof
  ;  selspecies=['TRACER']
  ;  selfield=['Air Concentration']
  ;  sellevel=['Boundary layer']
  ;  selgrid='grid1'
  ;  plotfield,'${WORKDIR}',selgrid,/exact,$
  ;    selspecies=selspecies,selfield=selfield,sellevel=sellevel,$
  ;    plotpmsl=0,plotuv=0,group='time',gengif=0,$
  ;    multiplot=[0,1,1],plotheader=1,plotlegend=1,genanim=1,$
  ;    projection=1,namever=2
  ;  eof
  ;
  ; example call for NAME II file with group='species'
  ;  GRAPHDIR=/home/h03/apdg/NameIII/NameIIIDevVersion/Version5_3a/Code_IDLGraphics
  ;  WORKDIR=${DATADIR}/${RUNNAME}
  ;  cd $GRAPHDIR
  ;  idl<<eof
  ;  selgrid='grid1'
  ;  selspecies=['VOLCANIC_ASH_0.3','VOLCANIC_ASH_1.0','VOLCANIC_ASH_3.0','VOLCANIC_ASH_10.0']
  ;  selfield=['Air Concentration']
  ;  sellevel=['From FL000 - FL015']
  ;  plotfield,'${WORKDIR}',selgrid,/exact,selspecies=selspecies,selfield=selfield,$
  ;   sellevel=sellevel,gengif=0,genanim=0,group='species',multiplot=[0,2,2]
  ;  eof
  ;
  ; subroutine calls:
  ;  readfieldhead
  ...
  if(back eq 1 or back eq 4)then fieldfiles=reverse(fieldfiles)
    ...
    'time': begin
      ...
      for ifield=0,numfields-1 do begin
        ...
        for ispec=0,numspecies-1 do begin
          ...
          ; Added to get around the "/" character in plotting ppt
          
          if (field eq 'Precipitation Rate (mm/hr)') then begin
             fieldtxt = 'Precipitation Rate'
          endif else begin
             fieldtxt = field   
          endelse
          ...
          if (namever eq 3)then begin
            ...
            filetem='Plot_'+filetext+$
              strcompress(fieldtxt,/remove_all)+'_'+$
          ...
          file=strcompress(datadir+delim+filetem+'.ps',/remove_all)
          set_plot,'ps'
          ...
          for it=0,numfiles-1 do begin
            ...
            if(back ne 0 and back ne 4)then begin
              postitle=attribstr
            endif else begin
              if (namever eq 3)then begin
                ; NAME III
            ...
            plotfieldpos,lon1,lat1,dat1, position=position,type=type,$
              ...
              mapfeatures=mapfeatures,legmax=legmax
          ...
          if((gengif eq 1 or genjpg eq 1 or genanim eq 1 or genpng eq 1) and pc eq 0)then begin
