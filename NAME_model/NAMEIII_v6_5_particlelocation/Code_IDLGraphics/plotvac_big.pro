pro plotvac_big,datadir,gridname,zoom=zoom,singleps=singleps,filetext=filetext,$
...
;
; DBR Feb 2002
; MH 2005      - modification of plotvac to give larger output images
...
; map limits
  ...
  dxy=ymax-ymin
  case 1 of 
    (dxy ge 90): gridxy=10                   ; 30
    (dxy ge 50) and (dxy lt 90): gridxy=5    ; 20
    (dxy ge 20) and (dxy lt 50): gridxy=2    ; 10
    (dxy ge 10) and (dxy lt 20): gridxy=2    ; 5
    (dxy ge 5) and (dxy lt 10): gridxy=1     ; 2
    (dxy ge 0) and (dxy lt 5): gridxy=1      ; 1
  endcase
  res=gridxy
...
; plot location
  ...
  position(0,*)=[0.05,0.65,0.49,0.95]
  position(1,*)=[0.51,0.65,0.95,0.95]
  position(2,*)=[0.05,0.25,0.49,0.55]
  position(3,*)=[0.51,0.25,0.95,0.55]
...
; annotation    
  ...
  if(it mod 2 eq 0)then begin
    xyouts,0.25,0.61,'FL350 - FL550',/normal,alignment=0.5,charsize=xysize*1.2,color=axiscol 
    xyouts,0.75,0.61,'FL200 - FL350',/normal,alignment=0.5,charsize=xysize*1.2,color=axiscol 
    xyouts,0.25,0.21,'SURFACE - FL200',/normal,alignment=0.5,charsize=xysize*1.2,color=axiscol 
    xyouts,0.75,0.21,'SURFACE - FL550!c(composite)',/normal,alignment=0.5,$
       charsize=xysize*1.2,color=axiscol 
    xyouts,0.5,0.13,'Valid '+head1(6),/normal,alignment=0.5,charsize=xysize*1.1,color=axiscol 
    xyouts,0.5,0.57,'Lat-Long grid spacing ='+string(gridxy,format='(i4)')+' degrees',$
           /normal,alignment=0.5,charsize=xysize*0.9,color=axiscol 
  ...
  xyouts,0.05,0.1,modtitle(4),/normal,charsize=xysize*0.9,color=axiscol  
  xyouts,0.05,0.08,modtitle(5),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.05,0.06,modtitle(6),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.05,0.04,modtitle(7),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.05,0.02,modtitle(8),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.6,0.1,'Pollutant: '+head1(1),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.6,0.08,modtitle(3),/normal,charsize=xysize*0.9,color=axiscol
  xyouts,0.6,0.06,modtitle(2),/normal,charsize=xysize*0.9,color=axiscol

  xyouts,0.6,0.04,'Threshold conc: '+$
        string(critconc,format='(e10.2)')+'g/m!e3!n',/normal,$
	charsize=xysize*0.9,color=axiscol 

  xyouts,0.6,0.02,'Summit height: '+$
        string(fix(sumht),format='(i6)')+'m',/normal,$
	charsize=xysize*0.9,color=axiscol 

  xyouts,0.5,0.0,'Met Office Crown copyright',$
	/normal,alignment=0.5,charsize=xysize*0.7,color=axiscol 

  image=read_image('MO_Master_B.jpg')
  loadct,0
  tv,image,0.83,0.92,true=1,xsize=0.14,/normal
  tek_color
