...
def convertCAMSaltitude(ds, species='co2', version='latest'):
    '''
...
def interpheight(nesw, fp_height, lonorlat, species='co2', version='latest', reverse=None, variables=None):
    """
...
def interplonlat(nesw, fp_lonorlat, lonorlat, species='co2', reverse=None, version='latest', variables=None, verbose=False):
    """
...
def bc_filename(domain, start_date, species='co2', from_climatology=False):
    '''
    ...
    if species=='co2':
        date_str = dt.strptime(start_date, "%Y-%m-%d").strftime('%Y-%m-%d')
...
def write_CAMS_BC_tonetcdf(vmr_n, vmr_e, vmr_s, vmr_w, st_date, domain, outdir, gridsize,
                           species='co2', version='latest', variables=None, from_climatology=False):
    """
...
def create_CAMS_BC(ds, 
    ...
    # Monthly averaged data
        ...
        tarray = ds[variables['time']]
        for i in range(0, len(tarray)-1):
            date = np.datetime_as_string(tarray[i])
            print('new',date)
            t_start_interval = tarray[i]
            t_end_interval = tarray[i+1]
            df = ds.sel(time=slice(t_start_interval, t_end_interval))
            df = df.mean(variables['time'])

            # find the correct unit conversion between mol/mol and species specific parts-per- units
            if not test:
                conversion = concentration(species_info[species.upper()]['units'])
                df[species].values *= conversion
            else:
                if verbose: print('Warning: testing, units will not be converted')

            # Select the gridcells closest to the edges of the  domain and make sure outside of fp
            lat_n = (np.abs(df.coords[variables['lat']].values - max(fp_lat))).argmin()
            if df.coords[variables['lat']].values[lat_n] < np.max(fp_lat) and lat_n != 0:
                lat_n -= 1
            lat_s = (np.abs(df.coords[variables['lat']].values - min(fp_lat))).argmin()
            if df.coords[variables['lat']].values[lat_s] > np.min(fp_lat) and lat_s != (len(df.coords[variables['lat']].values)-1):
                lat_s += 1
            lon_e = (np.abs(df.coords[variables['lon']].values - max(fp_lon))).argmin()
            if df.coords[variables['lon']].values[lon_e] < max(fp_lon) and lon_e != (len(df.coords[variables['lon']].values)-1):
                lon_e += 1
            lon_w = (np.abs(df.coords[variables['lon']].values - min(fp_lon))).argmin()
            if df.coords[variables['lon']].values[lon_w] > min(fp_lon) and lon_w != 0:
                lon_e -= 1
            # Cut to these and then interpolate
            north = df.sel(latitude  = df.coords[variables['lat']][lat_n],
                           longitude = slice(df.coords[variables['lon']][lon_w], df.coords[variables['lon']][lon_e])).drop_vars([variables['lat']])
            south = df.sel(latitude  = df.coords[variables['lat']][lat_s],
                           longitude = slice(df.coords[variables['lon']][lon_w], df.coords[variables['lon']][lon_e])).drop_vars([variables['lat']])
            east = df.sel(longitude = df.coords[variables['lon']][lon_e],
                           latitude  = slice(df.coords[variables['lat']][lat_s], df.coords[variables['lat']][lat_n])).drop_vars([variables['lon']])
            west = df.sel(longitude = df.coords[variables['lon']][lon_w],
                       latitude  = slice(df.coords[variables['lat']][lat_s], df.coords[variables['lat']][lat_n])).drop_vars([variables['lon']])
