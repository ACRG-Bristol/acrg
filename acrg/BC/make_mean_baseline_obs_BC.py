...
def main(
    ...
    file_name = f"{species}_BC_mhd_clean_sector_{initial_year}-{initial_year + n_years}.nc"
    ...
    if uncert_output_dir:
        ...
        uncert_file_path = uncert_path / f"{species}_BC_mhd_clean_sector_uncert_{initial_year}-{initial_year + n_years}.csv"
    ...
    if standardise:
        ...
        standardise_bc(file_path, species, bc_input, "EUROPE", period=freq, store=output_store, if_exists='new', save_current='yes')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-store", type=str)
    parser.add_argument("--obs-store", type=str)
    parser.add_argument("--fp-store", type=str)
