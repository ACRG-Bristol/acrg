...
cams_directory_default = os.path.join(data_path, 'ECMWF_CAMS', 'N2O/')

# latest version is currently v21, update as necessary
latest_version = 'v22r1'

# default variable names in the CAMS files for each species and CAMS versions
default_inputs = {'n2o': {'v22r1': {'altitude': 'altitude',
...
default_inputs['n2o']['latest'] = default_inputs['n2o'][latest_version]
...
def readCAMSInversion(start, end, species='n2o', cams_directory=cams_directory_default,
...
def convertCAMSaltitude(ds, species='n2o', version='latest'):
    '''
    ...
    hlevels = ds[variables['level']].values
    h_min = hlevels[0]
    h_max = hlevels[-1]

    low_alt = ds[variables['altitude']].sel(**{variables['level']:slice(h_min,h_max-1)})
    high_alt = ds[variables['altitude']].sel(**{variables['level']:slice(h_min+1,h_max)})
...
#    high_alt = ds[variables['altitude']].sel(**{variables['hlevel']:slice(h_min+1,h_max)})
    ...
    z_dims = tuple([dim if dim!=variables['level'] else "level" for dim in ds[variables['altitude']].dims])
    print(z.data.shape)   
...
def interpheight(nesw, fp_height, lonorlat, species='ch4', version='latest', reverse=None, variables=None):
    ...
    EDIT: (14 feb. 2023, ES): Swapped 'z' to 'altitude' for N2O    
    """
    version = f'v{version}' if not isinstance(version, str) else version
    # get default variable names
    variables = default_inputs[species][version] if variables is None else variables
    ...
    if reverse is None:
        ...
        print('THESE ARE THE ZCOORDS', z_coords)
        z_0 = nesw[variables['altitude']].sel(**{variables['level']:z_coords[0]}).values[0]
        z_1 = nesw[variables['altitude']].sel(**{variables['level']:z_coords[1]}).values[0]
...
#        z_1 = nesw[variables['z']].sel(**{variables['level']:z_coords[1]}).values[0]
    ...
    for jj in range(len(nesw[variables['altitude']][0,:])):
        if reverse == True:
            interp[:,jj] = np.interp(fp_height, nesw[variables['altitude']][:,jj][::-1], nesw[species][:,jj][::-1]).astype(float)
            # interp[:,jj] = np.interp(fp_height, nesw[variables['altitude']][:,jj][::-1], nesw[species][:,jj][::-1]).astype(np.float)
        elif reverse == False:
            interp[:,jj] = np.interp(fp_height, nesw[variables['altitude']][:,jj], nesw[species][:,jj]).astype(float)
            # interp[:,jj] = np.interp(fp_height, nesw[variables['altitude']][:,jj], nesw[species][:,jj]).astype(np.float)
    
    ds2 = xr.DataArray(interp, coords=[fp_height, nesw[lonorlat].values ], dims=['height', lonorlat])
    ds2 = ds2.to_dataset(name=species)
    return ds2

def interplonlat(nesw, fp_lonorlat, lonorlat, species='n2o', reverse=None, version='latest', variables=None, verbose=False):
    """
...
def bc_filename(domain, start_date, species='n2o', from_climatology=False):
    '''
...
def write_CAMS_BC_tonetcdf(vmr_n, vmr_e, vmr_s, vmr_w, st_date, domain, outdir, gridsize,
                           species='n2o', version='latest', variables=None, from_climatology=False):
    """
...
def makeCAMSBC(domain, start, end,
    ...
    # !!!!!!!!TO DO USE info on the domain directly and not take it from a random file!! change that in domain_volume!
    ...
    cams_ds = readCAMSInversion(clim_start, clim_end, cams_directory = cams_directory,
                                version=cams_version, species=species)

    print(cams_ds)

    # reverse N2O latitude axis to match for CH4
    cams_ds['latitude'] = np.flip(cams_ds['latitude'])
    cams_ds['n2o'] = np.flip(cams_ds['n2o'],axis=2)
    cams_ds['ps'] = np.flip(cams_ds['ps'],axis=1)
    cams_ds['altitude'] = np.flip(cams_ds['altitude'],axis=2)
    cams_ds['pressure'] = np.flip(cams_ds['pressure'],axis=2)
