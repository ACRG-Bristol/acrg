...
def convertCAMSaltitude(ds, species="co2", version="latest"):
    ...
    z = low_alt.values + altitude_diff/2.
...
def write_CAMS_BC_tonetcdf(vmr_n,
    ...
    BC_edges.to_netcdf(path=os.path.join(outdir, filename),  mode="w")
...
def create_CAMS_BC(ds,
    ...
    lat_grid = np.mean(ds[variables["lat"]].values[1:] - ds[variables["lat"]].values[:-1])
    lon_grid = np.mean(ds[variables["lon"]].values[1:] - ds[variables["lon"]].values[:-1])
...
#        ds = ds 
#    else:
#        raise KeyError(f"{time_res} not recognised. Please use one of 'monthy' or 'highest' ")
