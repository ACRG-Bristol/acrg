...
def boundary_conditions(domain, species, start = None, end = None, bc_directory=None):
    ...
    """
    print("finding boundary conditions")
    print("Note!!!! I have edited this to only get files for the year running, or 2016 otherwise (for speed!)")
    ...
    filenames = os.path.join(bc_directory,domain,f"{species.lower()}_*_{year}*.nc")
...
def basis(domain, basis_case, basis_directory = None):
    ...
    """
    print("4. in name.py basis")
    print(basis_directory, basis_case)
    ...
    print("in name.py", basis_directory, file_path, files)

    ## adding this to handle basis files for other months being created triggering an error
    for f in files:
        try:    
            ...
            for site_ds in data[site]:
                ...
                if not all(s==scales_list[0] for s in scales_list):
                    ...
                    for i in scales_list:
                        if isinstance(i,list): rt.extend(np.flatten(i))
                    else: 
                        rt.append(i)
                    scales[site] = rt
                else:
    ...
    for site in sites:
        ...
        for si, source in enumerate(flux_sources):
            ...
            if source in list(basis_case.keys()):
                ...
                print("3. in fp_sensitivity,", basis_directory, basis_case)
