...
import acrg.hbmcmc.inversion_pymc3_elena as mcmc
...
def fixedbasisMCMC(species, sites, domain, meas_period, start_date, 
    ...
    fp_all = name.footprints_data_merge(data, domain=domain, met_model = met_model, calc_bc=True, 
                                        ...
                                        emissions_name=emissions_name, site_modifier=site_modifier)
    ...
    #quadtree_f = True
    print("1. QUAD TREE BASIS IS", quadtree_basis)
    print(fp_basis_case)
    ...
    if quadtree_basis:
        if fp_basis_case != None:
            ...
            print("2. in if quadtreebasis = None")
    ...
    print("done with that, onto fp_sensitivity")

    print(basis_directory, fp_basis_case)
    ...
    Hx = np.zeros(0)
    Y = np.zeros(0)
    print(f"line 259 Y has shape {np.shape(Y)}")
