...
def fixedbasisMCMC_no_bcs(species, sites, domain, meas_period, start_date, 
                   end_date, outputpath, outputname,
                   met_model = None,
                   species_footprint = None,
                   HiTRes = False,
                   xprior={"pdf":"lognormal", "mu":1, "sd":1},
                   sigprior={"pdf":"uniform", "lower":0.5, "upper":3},
                   bc_type = "Percentile", percentile = 10, 
                   bc_directory = None, bc_filename = None, 
                   offsetprior={"pdf":"normal", "mu":0, "sd":1},
                   nit=2.5e5, burn=50000, tune=1.25e5, nchain=2,
                   emissions_name=None, inlet=None, fpheight=None, instrument=None, 
    ...
    """
    Script to run hierarchical Bayesian MCMC for inference of emissions using
    pymc3 to solve the inverse problem. For inversion with no boundary conditions.
    Background must be pre-subtracted from data.  
    ...
    #Run Pymc3 inversion
    xouts, sigouts, Ytrace, convergence, step1, step2 = mcmc.inferpymc3_no_bcs(Hx, Y, error, siteindicator, sigma_freq_index,
           xprior, sigprior, nit, burn, tune, nchain, sigma_per_site, offsetprior=offsetprior, add_offset=add_offset, verbose=verbose)
    #Process and save inversion output
    mcmc.inferpymc3_postprocessouts_no_bcs(xouts, sigouts, convergence, 
                               Hx, Y, error, Ytrace,
                               step1, step2, 
