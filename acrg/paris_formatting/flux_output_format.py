...
ds0 = xr.open_dataset(
    "/home/brendan/Documents/inversions/paris_ch4_tests/CH4_EUROPE_PARIS_dec2023_weighted_2021-02-01.nc"
)
...
ds0["uYaprioriBC"] = xr.DataArray(y_aprioribc_u, coords=[ds0.nmeasure], dims=["nmeasure"], name="qYaprioriBC")
...
time_vars = [
    tv for tv in time_vars if all(x not in tv for x in ["mode", "median", "off", "sensitivity", "sigma"])
]
...
data_var_attrs = {k: {j: w for j, w in v.items() if "FillValue" not in j} for k, v in data_var_attrs.items()}
