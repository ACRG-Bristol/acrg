...
rjmcmc, para_temp, hbayes, uncorr, corr,evencorr, &
sigma_clon, sigma_clat, nmeasure_site, nsite_max, & 
...
! NEED TO MODULARIZE
! WRITE SO THAT FIXED BASIS FUNCTIONS CAN BE USED 
! WRITE DIFFENT SUBROUTINES FOR DIFFERENT STEPS SO SAME FORTRAN SCRIPT CAN BE USED


! EACH UPDATE ALREADY A DIFFERENT SUBROUTINE
! MIGHT BE A GOOD IDEA TO PUT INTO SEPARATE FILE ANYWAY. NEED TO ESTABLISH WHICH SUBROUTINES ARE DIFFERENT FOR EACH CODE TYPE. 

! x_hparam_update - no change needed 
! x_update - done
! Birth - done
! Death - done 
! Move - done
! Sigma_y_update - done
! Tau_update - corr and evencorr only - add if statement
! y_update - evencorr only - add if statement

! 10/04/18
! Still need to add evencorr options as well
! Need to comment file to greater extent to explain everything that's going on.

! Still need to sort possible bug in x_update (and x_hparam_update) - DONE, NOT A BUG!!
...
REAL y_hparam1
REAL y_hparam2
LOGICAL rjmcmc, hbayes, para_temp
LOGICAL corr, uncorr, evencorr
! Input arrays
REAL stepsize(nIC1)
...
REAL n0_temp(nmeasure), sigma_y_temp(nmeasure)
REAL sigma_yinv(nmeasure), C(nmeasure), y_error_temp(nmeasure)
! SUBROUTINE INPUTS
...
!f2py intent(in) sigma_model, sigma_measure, nmeasure_site, nsite_max
...
!f2py intent(in) hbayes, rjmcmc, para_temp, corr, uncorr, evencorr
...
IF (.NOT. corr .AND. .NOT. evencorr) THEN            ! If neither corr or evencorr are true then make sure uncorr is true to run as default
    uncorr = .TRUE.
ENDIF
...
rej_tau_batch=0
 ...
 DO jj=1,ydim2   
        y_error_temp(R_indices(:,jj)) = sigma_model_ap(jj)*error_structure(R_indices(:,jj))    
 ENDDO  
...
ELSE IF (corr) THEN
    ...
    DO ibeta = 1,nbeta
       ...
       n0T(ibeta) = n0T_temp
    ENDDO
ELSEIF (evencorr) THEN

  sigma_yinv = 1./sigma_y_temp

  q_small = exp((-1.)*deltatime(1,2)/tau(1,1))   ! Assumes all tau start from the same value
...
ENDIF   ! uncorr/corr/evencorr statement

! Options: 
! rjmcmc on/off
! hbmcmc on/off
! uncorr/corr/evencorr
...
DO it=1,(nIt+burn_in)
 ...
 !SELECT CASE (inv_type)
      ...
      IF (corr) THEN
           IF (rjmcmc .AND. hbayes) THEN                 ! rjmcmc needs to be Boolean
               ! If doing reversible jump and hierarchical:       
               remain_it= modulo(it,6)+1
           ELSEIF ( (.NOT. rjmcmc) .AND. hbayes) THEN 
               ! If doing fixed dimension MCMC and hierarchical:      
               remain_it = modulo(it,3)+4
           ELSEIF ( rjmcmc .AND. ( .NOT. hbayes)) THEN 
               ! If doing reversible-jump MCMC and fixed hyper-parameters:      
               remain_it = modulo(it,4)+1
           ELSE
               ! If doing fixed-dimension MCMC and fixed hyper-parameters:      
               remain_it = 4
           ENDIF
...
!$OMP& shared(x,n0,n0T, k, pdf_param1, pdf_param2, h_agg, plon,plat, regions_v)
   do ibeta=1,nbeta
     ...
     if (para_temp .or. ibeta .EQ. 1) then 
       ...
       if (remain_it .EQ. 4) then              ! X UPDATE AND EMISSIONS HYPERPARAMETER UPDATE

            call x_update(uncorr,betaib,kib, xib, pdf_param1ib1,pdf_param2ib1, &
                         h_aggib,n0ib,n0Tib,sigma_yib, Rinvib, stepsize,&
       ...
       elseif (remain_it .EQ. 1) then       ! BIRTH
               ...
               call birth(uncorr, betaib,kib, xib, h_aggib,y,n0ib,n0Tib,sigma_yib, Rinvib, &
                          plonib, platib, regions_vib, lon,lat, & 
           ...
           elseif (remain_it .EQ. 2) then    ! DEATH
               ...
               call death(uncorr,betaib,kib, xib, h_aggib, y,n0ib,n0Tib,sigma_yib, Rinvib, &
                          plonib, platib, regions_vib, lon,lat, & 
           ...
           elseif (remain_it .EQ. 3) then    ! MOVE
                ...
                call move(uncorr, betaib,kib, xib, h_aggib, y,n0ib,n0Tib,sigma_yib, Rinvib, &
                         plonib, platib, regions_vib, lon,lat, & 
          ...
          elseif (remain_it .EQ. 5) then  ! X and Y hyper-parameters  UPDATE
              ...
              call  sigma_y_update(uncorr,betaib, sigma_modelib, sigma_measure, &
                         sigma_yib, error_structure, detvalib, &
           ...
           elseif (remain_it .EQ. 6) then  ! TAU_UPDATE

            ! if (betaib .EQ. 1.) then 
              IF (stepsize_tau .GT. 0.) THEN 
              call tau_update(corr, evencorr,betaib, tauib, sigma_yib, detvalib, detval_Qib, detval_Q_blockib, &
           ...
           elseif (remain_it .EQ. 7) then  ! Y_UPDATE
...
! Propose new values of pdf_param1 and pdf_param2
  ...
  pdf_param2_new = pdf_param2 + dpdf_param2
         ...
         if( (it .le. burn_in) .and. (modulo(it,500) .lt. 8)) then       ! Update stepsize once every 100 iterations. Modulo could be variable so allow felxibility.

         !if(it .le. burn_in .and. sum(accept_batch+reject_batch) .ge. 100*nIC1) then
             if (xi .LE. nIC) then
                 if(accept_batch(xi)+reject_batch(xi) .gt. 10) then          ! Only update stepsize if at least 10 values from which to calculate acceptance ratio.
                 ...
                 if(accept_batch(nIC1)+reject_batch(nIC1) .gt. 10) then    
...
SUBROUTINE x_update(uncorr,beta,k, x, pdf_param1_all,pdf_param2_all,  &
h_agg,n0,n0T,sigma_y, Rinv, stepsize,&
accept_batch, reject_batch, x_pdf_all, it, burn_in, nIC, kICmax, nmeasure, nIC1, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         if(beta .eq. 1 .and. it .le. burn_in .and. modulo(it,500) .lt. 8) then
         !if(beta .eq. 1 .and. it .le. burn_in .and. sum(accept_batch+reject_batch) .ge. 100*nIC1) then
             !if (xi .EQ. 1) then
             if (xi .LE. nIC) then
                 !write(*,*) accept_batch
                 if(accept_batch(xi)+reject_batch(xi) .gt. 10) then
                     accep_prob(xi) = real(accept_batch(xi))/(accept_batch(xi)+reject_batch(xi))
...
SUBROUTINE birth(uncorr,beta,k, x, h_agg,y,n0,n0T,sigma_y, Rinv, plon, plat, regions_v, lon,lat, & 
...
SUBROUTINE death(uncorr, beta,k, x, h_agg,y,n0,n0T,sigma_y, Rinv, plon, plat, regions_v, lon,lat, & 
...
SUBROUTINE move(uncorr, beta,k, x, h_agg, y,n0,n0T,sigma_y, Rinv, &
plon, plat, regions_v, lon,lat, & 
h_v, lonmin, lonmax, latmin,latmax, sigma_clon, sigma_clat, it, &
burn_in, nIC, kICmax, kIC, kmax, nmeasure, Ngrid,nlon,nlat, &
...
SUBROUTINE sigma_y_update(uncorr,beta, sigma_model_current, sigma_measure, sigma_y_current, &
error_structure, detval_current, detval_Q, Rinv_current, Qinv, &
...
! So if corr use Rinv and Qinv detval_Q etc.;  if not then just use sigma_y 

! Set this up with input options: corr, uncorr, (evencorr)
...
INTEGER ti
REAL autocorr_vec(nmeasure)
REAL sigma_yinv_new(nmeasure)
...
reject=0
 ...
 !do yi=1,dim2
    call random_number(u)   
    yi = FLOOR(dim2*u)+1           ! Select an index of sigma_model to update 
		
    ! Generate new value of sigma_y
    dsigma_y = random_normal()*stepsize_sigma_y(yi)    
    sigma_model_new = sigma_model_current(yi) + dsigma_y       ! Specify new signa_model value
    ...
    do jj=1,dim2   
        y_error_new(R_indices(:,jj)) = sigma_model_current(jj)*error_structure(R_indices(:,jj))    ! Error structure allows a pre-designated structure to be scaled
    enddo  

    ! Change indices of sigma_y that correspond to sigma_model index - mapped through R_indices
    y_error_new(R_indices(:,yi)) = sigma_model_new*error_structure(R_indices(:,yi))  ! R_indices = array of indices to which each sigma_y applies 			   
    sigma_y_new=sqrt(y_error_new**2 + sigma_measure**2)   

    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! If doing uncorrelated inversion:
    ...
    !call sgemm ('N', 'N', nmeasure, 1, nmeasure, aa, Rinv_new, nmeasure, n0, nmeasure, bb, C, nmeasure)    ! Matrix multiplication (faster than matmul)
    ...
    !if(beta .eq. 1 .and. it .le. burn_in .and. sum(accept_batch+reject_batch) .ge. 100) then
        ...
        do ii=1,dim2 
          if(accept_batch(ii)+reject_batch(ii) .gt. 10) then          
...
SUBROUTINE tau_update(corr, evencorr, beta, tau_current, sigma_y,  &
...
! Compute P1 for new value of tau
   ...
   call calc_pdf(tau_new(yi), tau_hparam1, tau_hparam2, tau_pdf, p1_tau)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       q_small = exp(-1.*deltatime(1,2)/tau_new(yi))    ! deltatime = single number; q_small=single number
    ...
    ELSE
       WRITE(*,*) "Neither corr or evencorr is specified"
       STOP
    ENDIF
...
!endif
 ...
 if(beta .eq. 1 .and. it .le. burn_in .and. modulo(it,500) .lt. 8) then
       if (it .gt. 100) then
        av_acc =  max(0.01,1.0/sqrt(real(it)/500.))       
          if(accept_batch+reject_batch .gt. 10) then          
