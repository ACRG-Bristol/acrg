...
def met_satellite_split(met):
    ...
    if "label" in met[0].columns.values:
        if len(met[0]["label"][0]) >= min_label_digit:
            point_num = np.unique([label[:-lev_digit] for label in met[0]["label"].values])
            met = [met[0][met[0]["label"].str.startswith(num)] for num in point_num]
...
def particle_locations(particle_file, time, lats, lons, levs, heights, id_is_lev = False,
    ...
    if satellite:
        npoints = len(set(np.array(df["Id"])))
...
def footprint_concatenate(fields_prefix, 
    ...
    if satellite:
        ...
        fields_files = sorted(glob.glob(fields_prefix + "*" +
                             datestr + ".txt*"))
        #print fields_prefix + "*" + datestr + ".txt*"
    else:
        fields_files = sorted(glob.glob(fields_prefix + "*" +
                             datestr + "*.txt*"))
...
def satellite_vertical_profile(fp, satellite_obs_file, max_level):
    ...
    #print "pressure_weights",sat.pressure_weights.values
    #print "pressure weights squeezed",sat.pressure_weights.values.squeeze()
    #print "pressure weights squeezed, lower levels only",sat.pressure_weights.values.squeeze()[lower_levels]
...
#        print "var.reshape((1, 1) + var.shape)",out[variable]
#        print "shape",out[variable].shape
    ...
    for t in range(ntime):
        ...
        for variable in variables:
            ...
            if variable[0:2] == "pl":
                ...
                print "*** new fp value ***",variable,np.mean(out[variable][t]),np.max(out[variable][t]),np.sum(out[variable][t])
...
#        for variable in variables:
#            a = fp_combined.isel(time = [t])[variable]
#            print "* fp_combined value before *",variable,a,np.max(a.values),np.min(a.values),np.mean(a.values)
#            fp_combined.isel(time = [t])[variable] = out[variable]
#            a = fp_combined.isel(time = [t])[variable]
#            print "*** new fp_combined[dict(time = t)][variable] ***",variable,a,np.max(a.values),np.min(a.values),np.mean(a.values)
