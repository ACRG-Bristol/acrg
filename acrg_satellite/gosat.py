...
def name_pressure_match(ds,pressure_domain,columns=["latitude","longitude","time"],p_column="surface_pressure",
    ...
    for i,la,lo,t in zip(list(range(len(lat))),lat,lon,time):
        if not day_template or t <= end_datetime:
            ...
            max_offset_days = np.timedelta64( np.timedelta64((t - end_datetime),'D') + np.timedelta64(1, "D") ,"D")#.astype(int)+1
#            max_offset_days = np.timedelta64((t - end_datetime),'D').astype(int)+1
            
            pressure_NAME_offset = pressure_NAME.copy(deep=True)
            pressure_NAME_offset[columns[2]] += max_offset_days #np.timedelta64(max_offset_days)# # Create copy of dataset with date offset to date we're looking for.
            # 
